-------------change date in line 10 and file names in line 24 and 35-- Last run June 8, 2015 for May 31, 2015 data---------------
...
INSERT INTO tempDate  ([Mth]

           ,[Mth1]

           ,[Mth0]

           ,[Mth_1]) values(@Mth, @Mth1, @Mth0, @Mth_1) 
...
SELECT * INTO PQR_AMC FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','Excel 12.0;IMEX=1;Database=D:\OwnCloud\Grassland\Credit and Risk\Reports\Rawdata\KxlRpt20151130.xlsx','SELECT * FROM [20151130$]')
...
SELECT * INTO DEL_TEMP FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','Excel 12.0;IMEX=1;Database=D:\OwnCloud\Grassland\Credit and Risk\Reports\Rawdata\rptAtRisk20151130.xlsx','SELECT * FROM [20151130$]')
...
Alter table PQR_AMC add Balance money,current1 money, P1_5 money, P6_15 money, P16_30 money, P31_60 money, P61_90 money, P91_120 money, P121_150 money, P151_180 money, P180 money, [PAR>0] money , [PAR>5] money ,[PAR>15] money ,[PAR>30] money ,[PAR>60] money, PAR1to30 money, MOB int, MonthDisbursed int, MatExp int,MaturityExp datetime, loancycle varchar(5), Monthterm int, Monthobs int, NewRenew varchar(10), Client int,MonthDisbNo int, MonthNo int
...
Insert into TempTotal_Financial select LoanID as Lnnote, ClientID, LnOfcrName as LO, ActDisbDate as Disbursement_date, ApprvdDisb as LoanAmt , LastPmtDt,NrTerms as LoanTerm, Rschd,SchdPmtAmt as EMI,SchdType, Fund, Bank, PrevLns, MthSales, BusEquity, FamIncom,SiteVisitDt, CrdtComDt,DisbDt, EmpB4Ln, IntRateQt, LnCharLkp4 as Prod, TrmLth, Rnd, DisbFee, Street, City, PostCode, StMun, Region, Country, Branch, UrbRur,BirthDt,Gender, Ethn, MarStat,NrTotalDep, NrHouseDep, CtryOrig, Citizen,ID1Type,ID2Type,IndivLkp1,IndivLkp2,IndivLkp3,IndivStr1,IndivStr2,ClientType,LoanType,LnProd,LastSchdDt,Rschd2,[Observation Date],DPD,Balance, Current1, P1_5,P6_15,P16_30,P31_60,P61_90,P91_120,P121_150,P151_180,P180,[PAR>0],[PAR>5],[PAR>15],[PAR>30],[PAR>60],[PAR1to30],MOB,MonthDisbursed,MatExp,loancycle,Monthterm,Monthobs,MaturityExp,NewRenew,Client,MonthDisbNo,MonthNo,ClientName,' ' from PQR_AMC
